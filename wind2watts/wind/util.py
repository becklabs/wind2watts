...
def add_wind_data(
    ...
    TODO: Evaluate model performace when a mean of lat/lon is used for a given hour,
    vs the actual lat/lon
    """
    df = df.copy()
    ...
    for (year, month, day, hour), group in df.groupby(["year", #type: ignore
                                                       "month",
                                                       "day",
                                                       "hour"]): 
